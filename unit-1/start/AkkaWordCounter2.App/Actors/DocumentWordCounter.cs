...
public sealed class DocumentWordCounter : UntypedActor
{
    ...
    //https://petabridge.com/bootcamp/lessons/unit-1/unit-testing-akkadotnet/
    ...
    protected override void OnReceive(object message)
    {
        switch (message)
        {
            ...
            case EndOfDocumentReached endOfDocumentReached when endOfDocumentReached.DocumentId == _documentId:
                var output = new CountsTabulatedForDocument(
                    _documentId,
            ...
            case IWithDocumentId withDocumentId when withDocumentId.DocumentId != _documentId:
                _log.Warning("Recieved message for document {0} but I am responsible for dcoument {1}", withDocumentId.DocumentId, _documentId);
                break;
    ...
    private void Complete(object message)
    {
        ...
        {
            case FetchCounts:
                Sender.Tell(new CountsTabulatedForDocument(
                    _documentId,
                    _wordCounts.ToImmutableDictionary(x => x.Key, x => x.Value)));
                break;

            case IWithDocumentId withDocumentId when withDocumentId.DocumentId == _documentId:
                _log.Warning("Recieved message for document {0} but I have already completed processing", withDocumentId.DocumentId);
                break;

            case IWithDocumentId withDocumentId when withDocumentId.DocumentId != _documentId:
                _log.Warning("Recieved message for document {0} but I am responsible for document {1}", withDocumentId.DocumentId, _documentId);
                break;
